    ...
    public class EventBusRabbitMQ : BaseEventBus
    {
        ...
        public override void Publish(IntegrationEvent @event)
        {
            ...
            var policy = Policy.Handle<BrokerUnreachableException>()
                .Or<SocketException>()
                .WaitAndRetry(EventBusConfig.ConnectionRetryCount, retryAttemp => TimeSpan.FromSeconds(Math.Pow(2, retryAttemp)), (Exception, time) =>
                {
                    // log
                });
            ...
            policy.Execute(() =>
            {
                ...
                consumerChannel.QueueDeclare(queue: GetSubName(eventName), //queue check, yoksa olustur
                                        durable: true,
                                        exclusive: false,
                                        autoDelete: false,
                                        arguments: null);
        ...
        public override void Subscribe<T, TH>()
        {
            ...
            if (!SubsManager.HasSubscriptionsForEvent(eventName))
            {
                ...
                consumerChannel.QueueDeclare(queue: GetSubName(eventName), //queue olustur
                                        durable: true,
                                        exclusive: false,
                                        autoDelete: false,
                                        arguments: null);

                consumerChannel.QueueBind(queue: GetSubName(eventName),    //queue ve exchange birbirine bind et
                                        exchange: EventBusConfig.DefaultTopicName,
                                        routingKey: eventName);
            }
